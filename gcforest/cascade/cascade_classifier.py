...
def calc_auc(y_true, y_pred, name, prefix=""):
    ...
    LOGGER.info('{}AUC({})={:.2f}%'.format(prefix, name, auc))
    return auc
...
class CascadeClassifier(object):
    def __init__(self, ca_config):
        ...
        self.data_save_dir = ca_config.get("data_save_dirz", None)
    ...
    def fit_transform(self, X_groups_train, y_train, X_groups_test, y_test, stop_by_test=False, train_config=None):
        ...
        try:
            ...
            while 1:
                ...
                train_avg_auc = calc_auc(y_train,y_train_proba_li[:,1], 'layer_{} - train.classifier_average'.format(layer_id))
                train_acc_list.append(train_avg_auc)
                if n_tests > 0:
                    ...
                    test_avg_auc = calc_auc(y_test,y_test_proba_li[:,1], 'layer_{} - test.classifier_average'.format(layer_id))
                    test_acc_list.append(test_avg_auc)
                else:
